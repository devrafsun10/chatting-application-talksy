...
const Sidebar = ({ active }) => {
  ...
  return (
    <div className="bg-[#1E1E1E] font-primary  w-[186px] h-[180vh] m-[35px] rounded-lg">
      ...
      <div
        className={`relative after:absolute after:content-[''] after:top-0 after:left-0 after:w-[167px] after:h-full ${
      ...
      >
        <Link to="/">
          <GoHome
            className={`text-5xl ${
              active == "home" ? "font-primary" : "text-white"
            } `}
          />
        </Link>
      </div>
      ...
      >
        <Link to="/msg">
          <AiTwotoneMessage
            className={`text-5xl ${active == "message" ? "font-primary" : ""} `}
          />
        </Link>
      </div>
      <div
        className={`relative after:absolute after:content-[''] after:top-0 after:left-0 after:w-[167px] after:h-full ${
          active == "setting" ? "after:bg-white" : "after:bg-transparent"
        }  after:z-[-1] z-1 after:ml-[20px] after:rounded-lg 
        before:absolute before:contemt-[''] before:top-0 before:right-[0px] before:h-full before:w-[10px] before:bg-[#1E1E1E] before:rounded-tl-lg before:rounded-bl-lg before:shadow-2xs/90 before:shadow-[-2px_0px_4px_0px_rgba(0,0,0,0.25)] mt-[78px] py-[20px] flex justify-center cursor-pointer`}
      >
        <Link to="/setting">
        <IoSettingsOutline className={`text-5xl ${active == "setting" ? "font-primary" : ""} `} />
        </Link>
      </div>
      <div className=" py-[200px] flex justify-center cursor-pointer   ">
        <ImExit
          onClick={handleLogout}
          className="text-5xl text-white font-primary"
        />
      </div>
    </div>
  );
